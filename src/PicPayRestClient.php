...
class PicPayRestClient
{
    const API_BASE_URL = 'https://appws.picpay.com/ecommerce/public';
    ...
    private static function build_request($request)
    {
        ...
        curl_setopt($connect, CURLOPT_USERAGENT, 'PicPay PHP v' . PicPay::VERSION);
    ...
    private static function exec($request)
    {
        ...
        if ($api_result === false) {
            throw new PicPayException(curl_error($connect));
        }
        ...
        if ($response['status'] >= 400) {
            ...
            throw new PicPayException($message, $response['status']);
        }
